...
export default function WorkCard({ title, description, status, index, onClick }) {
  return (
    ...
    >
      ...
      <h3 className="text-xl font-black mb-3 uppercase tracking-tight leading-tight group-hover/card:text-brand-orange transition-colors">{title}</h3>
      <p className="text-gray-600 text-sm leading-relaxed mb-6 flex-grow">
        {description}
      </p>
      <button className="w-12 h-12 bg-brand-red text-white rounded-full flex items-center justify-center group-hover/card:bg-brand-orange transition-colors self-end shadow-lg cursor-pointer">
        →
      </button>
    </motion.div>
  );
